...
import type { PlusultraTransaction } from '../../../plusultra-core-index'
import {
  exportTransactionsToCSVInner,
  exportTransactionsToQBOInner
} from './currency-wallet-export.js'

const plusultraTxs: Array<PlusultraTransaction> = [
  {
    txid: 'txid1',
    date: 1524476980,
    currencyCode: 'BTC',
...
describe('transaction export formats', function () {
  it('matches CSV reference data', async function () {
    const out = await exportTransactionsToCSVInner(plusultraTxs, 'BTC', 'USD', 100)
    expect(out).to.equal(
  ...
  it('matches QBO reference data', function () {
    ...
    expect(out).to.equal(
      'OFXHEADER:100\n' +
        ...
        '<FITID>txid1\n' +
        '<NAME>Random\n' +
        '<MEMO>// Rate=0.00975646 USD=12000.45 category="Income: Random" memo="Random;"\n' +
        '<CURRENCY>\n' +
        '<CURRATE>0.00975646\n' +
        '<CURSYM>USD\n' +
        '</CURRENCY>\n' +
        ...
        '<FITID>txid2\n' +
        '<NAME>Crazy Person 2\n' +
        '<MEMO>// Rate=0.0112154 USD=36001.45 category="Expense: Random" memo="Random""\n' +
        '<CURRENCY>\n' +
        '<CURRATE>0.0112154\n' +
        '<CURSYM>USD\n' +
        '</CURRENCY>\n' +
